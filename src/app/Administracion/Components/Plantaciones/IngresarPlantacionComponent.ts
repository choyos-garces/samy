...
export class IngresarPlantacionComponent {
    ...
    ngOnInit() {
        ...
        this._administracionService.getEmpresas(0).subscribe(productores => {
            this.propietarios = productores;
        ...
        this._opcionesService.getProductosPlantacion().subscribe(productos => this.productos = productos);
    ...
    objectToFormControl(id : any, collection : string, control : string) : void {
        ...
        (<Control>this.ingresoPlantacion.controls[control]).updateValue((results.length == 1) ? results[0] : null, {});
    }
}